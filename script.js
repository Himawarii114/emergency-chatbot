...
function disasterAwareness(text) {
  ...
  riskScore += 20;
  respond("Landslides can be dangerous. Move away from slopes or unstable ground if possible.");
}

if (text.includes("building shaking") || text.includes("ceiling falling")) {
  riskScore += 20;
  respond("If indoors, drop, cover, and hold on. Protect your head and neck.");
}

if (text.includes("water entering") || text.includes("road flooded")) {
  riskScore += 15;
  respond("Avoid walking or driving through flood water. Move to higher ground if you can.");
}
...
function aiInstructions(text) {
  const t = text.toLowerCase();

  // Fear / panic
  if (
    t.includes("panic") || t.includes("panicking") ||
    t.includes("scared") || t.includes("afraid") ||
    t.includes("anxious") || t.includes("terrified") ||
    t.includes("nervous")
  ) {
    return "I know this feels overwhelming. Take a slow breath with me. You’re not alone. Stay where you are if it’s safe.";
  }

  // Being followed / threatened
  if (
    t.includes("follow") || t.includes("following") ||
    t.includes("stalk") || t.includes("stalking") ||
    t.includes("threat") || t.includes("threatened") ||
    t.includes("someone behind me")
  ) {
    return "If possible, move to a public, well-lit place. Avoid isolated areas. Keep your phone silent and stay alert.";
  }

  // Injury / pain
  if (
    t.includes("bleeding") || t.includes("blood") ||
    t.includes("injured") || t.includes("hurt") ||
    t.includes("pain") || t.includes("wound")
  ) {
    return "If there is bleeding, apply firm pressure with a clean cloth. Try to stay still and conserve energy.";
  }

  // Fire / smoke
  if (
    t.includes("fire") || t.includes("burning") ||
    t.includes("flames") || t.includes("smoke")
  ) {
    return "Move away from smoke if you can. Stay low to the ground and cover your nose. Avoid elevators.";
  }

  // Disaster
  if (
    t.includes("earthquake") || t.includes("tremor") ||
    t.includes("flood") || t.includes("water rising") ||
    t.includes("cyclone") || t.includes("storm")
  ) {
    return "Try to move to a safer area if possible. Stay away from windows and unstable structures.";
  }

  return "I’m here with you. Please tell me what’s happening around you so I can guide you.";
}
...
function addImageToChat(imgURL) {
...
function aiKeywordRisk(text) {
  const t = text.toLowerCase();
  let extraRisk = 0;

  const dangerWords = [
    "panic","scared","afraid","anxious","threat","follow",
    "kidnap","attack","bleeding","hurt","injured",
    "fire","smoke","trapped","help"
  ];

  dangerWords.forEach(word => {
    if (t.includes(word)) extraRisk += 5;
  });
